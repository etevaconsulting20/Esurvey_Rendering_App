...
import { CustomStep, OptionsStep, TextStep,STextStep } from './steps_components';
...
import {cityList,interests} from './common/SearchList'
...
class ChatBot extends Component {
  ...
  onValueChange = event => {
    ...
    if(this.state.currentStep.isSerchable == true){
      //alert('show serched values');
    }
  };
  ...
  generateRenderedStepsById = () => {
    ...
    for (let i = 0, len = previousSteps.length; i < len; i += 1) {
      const { id, message, value, metadata,isSerchable } = previousSteps[i];
  ...
  modifyJson(step){
    let chatResponseObject;
    if(step.type==='Answer' && !!step.message)
    {
      // let mid=step.id;
      let chatResponseValue = [];
      let message = step.message;
      if(!Array.isArray(message)){
        chatResponseValue.push(message);
      }else{
        message.map((o,index)=>{
          chatResponseValue.push(o.label);
        })
      }
  ...
  triggerNextStep = data => {
    ...
    if (isEnd) {
      ...
      this.setState({ renderedSteps, currentStep, previousStep }, () => {
        if(currentStep.ComponentType == "MULTI_SELECT"){
          currentStep.ItemsList = _.map(currentStep.ItemsList,(o)=>(
            {...o,ischecked:false}
          ))
        }
        if (nextStep.user) {
  ...
  handleSubmitButton = () => {
    ...
    //check if multiselect component
    if(currentStep.ComponentType === 'MULTI_SELECT')
    {
      this.submitMultiselect(this.multiselectRef.current.state._selectedList);
  ...
  selectFromSearchList=(value)=>{
    this.setState({inputValue:value.label,isShowOptions:false});
    // let currentStep = {...this.state.currentStep,suser:true,SelectedOptions: [
    //   { value: 1, label: 'Number 1' },
    //   { value: 2, label: 'Number 2' },
    //   { value: 3, label: 'Number 3' },
    //   { value: 4, label: 'Number 4' },
    // ]}
    this.submitUserMessage(value.label);
  }

  submitMultiselect=(values)=>{
    //this.setState({inputValue:value});
    let currentStep = {...this.state.currentStep,suser:true,SelectedOptions: values}
    this.submitUserMessage(values,currentStep);
  }
  submitUserMessage = (value,newCurrentStep) => {
    let { defaultUserSettings, inputValue, previousSteps, renderedSteps } = this.state;
    if(!!value){
      inputValue = value;
    }
  ...
  renderStep = (step, index) => {
    ...
    const { options, component, asMessage,SelectedOptions } = step;
    const steps = this.generateRenderedStepsById();
    ...
    return (  
      <div className={`rsc ${className}`}>
        ...
        >
          ...
          > 
            {this.state.currentStep.ComponentType=='SINGLE_SELECT' && 
           <SearchinList selectFromSearchList={this.selectFromSearchList} serachList={this.state.currentStep.ItemsList} inputText={this.state.inputValue}></SearchinList>
          }
          {this.state.currentStep.ComponentType == 'MULTI_SELECT' && 
          <>
           <MultiselectList ref={this.multiselectRef} submitMultiselect={this.submitMultiselect} serachList={this.state.currentStep.ItemsList} inputText={this.state.inputValue}></MultiselectList>
           </>
          }
